<?php
  ...
  class AjaxController extends ControllerBase implements ContainerInjectionInterface {
    ...
    public function alumni_news() {
      ...
      return new JsonResponse(['loadMore' => $load_more_show, 'data' => $output, 'lim' => $nid+10]);
    }
    ...
    public function image_upload($arg=NULL) {
      ...
      if($user) {
        ...
        if($_FILES['file']['error'] == 0) {
          ...
          if($height != 200 && $width != 290) {
            $msg = 'Image dimension should be 290px by 200px';
            $error = '1';
          }
          ...
          if($filesize > 2) {
            $error = '1';
            $msg = 'File exceeds the max allowed size - 2 MB.';
          }
          if($error == '0') {
      ...
      if(isset($_POST['return']) && !empty($_POST['return']) || $arg == 'job_apply'){
        if($error == 1) {
          return json_encode(array('error' => 1, 'msg' => $msg));
        } else {
          return json_encode(array('error' => 0, 'msg' => $fid));
        }
      } else {
    ...
    public function user_login() {
      ...
      if($actual_token == $token) {
        ...
        $query = \Drupal::database()->select('users_field_data', 'ufd');
        $query->fields('ufd', ['uid','access','login','pass','status']);
        $query->condition('ufd.mail', $email);
        $query->range(0, 1);
        ...
        if(!$row) {
          ...
          if($row['status'] == 0) {
            $mesg[] = 'confrm-Registration Confirmation Required-You need to click on the Confirmation Link sent to your email to Log in.';
            $error  = true;
          } else {
    ...
    public function forgot_password() {
      ...
      if(!$error) {
        ...
        if($uid) {
          ...
          $mail_url = $base_url.'/reset-password/'.$uid.'/'.$request_time.'/'.str_replace('/', '~', $pass);
          $site_mail = \Drupal::config('system.site')->get('mail');
          ...
          $mesg[] = 'fpEmail-Inserted Email ID is not registered.';
          $error = true;
        } 
      }
      return new JsonResponse(['mesg' => $mesg, 'status' => $status, 'tok' => $rnd_num]); 
    ...
    public function apply_job(){
      ...
      if($_POST['irma_profile']==0){
        $return = json_decode($this->image_upload('job_apply'));
        if($return->error == 1) {
          $message = $return->msg;
          $db_write = false;
        } else {
          $fid = $return->msg;
          if(!empty($result)){
            $query = \Drupal::database()->update('apply_irma_profile');
            $query->fields([
              'fid' => $fid,
              'irma_profile' => $_POST['irma_profile'],
