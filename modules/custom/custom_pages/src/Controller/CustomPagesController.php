<?php
  ...
  class CustomPagesController extends ControllerBase {
    ...
    public function my_account($arg) {
      ...
      if($arg != NULL) {
        ...
        if(strpos($arg, '-') === false) {
          ...
          if($uuid != $myUid) {
            $_SESSION['irma']['mesg'] = 'invalid';
            return new \Symfony\Component\HttpFoundation\RedirectResponse(\Drupal::url('custom_pages.user_authenticate'));
            //return new RedirectResponse(\Drupal::url('<front>'));
          } else {
            if($current_user == 0 || $current_user == $uid) {
              if($user->status->value != 0) {
                if($current_user == $uid) {
                  $_SESSION['irma']['mesg'] = 'logged_in';
                } else {
                  $_SESSION['irma']['mesg'] = 'already activated';
                }
                return new \Symfony\Component\HttpFoundation\RedirectResponse(\Drupal::url('custom_pages.user_authenticate'));
                //return new RedirectResponse(\Drupal::url('<front>'));
                ...
                $query->execute();
                $_SESSION['irma']['mesg'] = 'success';
                return new \Symfony\Component\HttpFoundation\RedirectResponse(\Drupal::url('custom_pages.user_authenticate'));
                //return new RedirectResponse(\Drupal::url('custom_pages.user_authenticate'));
                //user_login_finalize($user);
                //$login = true;
              }
            } else {
              return new \Symfony\Component\HttpFoundation\RedirectResponse(\Drupal::url('custom_pages.user_authenticate'));
              $_SESSION['irma']['mesg'] = 'other';
              //return new RedirectResponse(\Drupal::url('<front>'));
            }
          }
        }
    ...
    public function user_authenticate() {
      if($_SESSION['irma']['mesg'] == 'success') {
        $mesg = 'User successfully activated. Click <a href="/" class="href">here</a> to go to the homepage.';
      }
      if($_SESSION['irma']['mesg'] == 'invalid') {
        $mesg = 'The URL is invalid. Click <a href="/" class="href">here</a> to go to the homepage.';
      }
      if($_SESSION['irma']['mesg'] == 'already activated') {
        $mesg = 'User is already activated. Click <a href="/" class="href">here</a> to go to the homepage.';
      }
      if($_SESSION['irma']['mesg'] == 'logged_in') {
        $mesg = 'User is already activated and logged in. Click <a href="/" class="href">here</a> to go to the homepage.';
      }
      if($_SESSION['irma']['mesg'] == 'other') {
        $mesg = 'User is already activated and logged in. Click <a href="/" class="href">here</a> to go to the homepage.';
      }
